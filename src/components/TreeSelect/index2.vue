<!--
  ...
  * @use： <el-select-tree :data="menuTree" v-model="value"  node-key="id" />
-->
<template>
    <el-select class="tree-select" ref="treeSelect" @visible-change="onPopperVisibleChange" v-model="selectedValue"
        ...
        <el-option v-for="item in listData" :key="item[defaultProps.value]" :label="item[defaultProps.label || 'label']" :value="item[defaultProps.value]"></el-option>
    </el-select>
</template>
...
export default {
    ...
    computed: {
        treeProps () {
            return {
                data: this.data,
                nodeKey: this.defaultProps.value,
...
<style scoped>
  ...
  .tree-select.single-select-tree  .el-tree-node.is-current > .el-tree-node__content > .el-tree-node__label {
      color: #409EFF;
      font-weight: 700;
  }
