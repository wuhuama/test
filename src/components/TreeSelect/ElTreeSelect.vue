<!-- 使用 <el-select-tree :data="menuTree" v-model="form.pid" :defaultProps="defaultProps" node-key="id" size="small" /> -->
...
export default {
    ...
    watch: {
        ...
        selectedValue: {
            handler: (newVal, oldVal) => {
                if (JSON.stringify(newVal) !== JSON.stringify(oldVal) && self.dropdownTree == null) {
                    self.$emit('input', newVal)
                    self.$emit('change', newVal)
                }
            }
        }
    }
